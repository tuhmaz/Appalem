...
import type { Comment, FileItem, Post } from '@/types/api';
import { stripHtml } from '@/utils/format';
import { normalizeBaseUrl, normalizeExternalUrl, stripApiSuffix } from '@/utils/url';

const POST_COMMENT_TYPE = 'App\\Models\\Post';
...
type PostDetailsNavigation = NativeStackNavigationProp<{
...
function normalizePostAttachments(post: Post | null): FileItem[] {
  if (!post) return [];
  const raw = (post as Post & { attachments?: unknown }).attachments;
...
function normalizePostContent(raw?: string) {
  if (!raw) return '';
...
export function PostDetailsScreen() {
  ...
  const navigation = useNavigation<PostDetailsNavigation>();
  ...
  const labels = useMemo(() => ({
    contentTitle: t('postContent'),
    noContent: t('noPostContent'),
    noImage: t('noImage'),
    category: t('category'),
    date: t('date'),
    views: t('views'),
    openAttachment: t('openAttachment'),
    noAttachments: t('noPostAttachments'),
  ...
  useEffect(() => {
    ...
    postService.show(route.params.postId)
      .then(data => {
    ...
    postService.incrementView(route.params.postId).catch(() => {});

    return () => {
      mounted = false;
    };
  }, [route.params.postId, country.id]);
  ...
  useEffect(() => {
    ...
    commentService.list(database, {
      ...
      commentable_type: POST_COMMENT_TYPE,
      per_page: 50,
    })
      .then(result => {
  ...
  const postImageUrl = resolveAssetUrl(post?.image_url || post?.image);
  const attachments = normalizePostAttachments(post);
  const normalizedBody = normalizePostContent(post?.content || post?.meta_description || '');
  const postViews = post?.views_count ?? post?.views;
  
  const contentTitle = labels.contentTitle;
  const contentBody = normalizedBody || labels.noContent;
  const dateValue = post?.created_at
    ? new Date(post.created_at).toLocaleDateString(locale === 'ar' ? 'ar-JO' : 'en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    })
  ...
  const handleCommentSubmit = useCallback(async () => {
    ...
    try {
      const created = await commentService.create(database, {
        ...
        commentable_id: post?.id || route.params.postId,
        commentable_type: POST_COMMENT_TYPE,
      });
  ...
  }, [commentBody, user, navigation, database, post?.id, route.params.postId, t]);

  const openAttachment = (file: FileItem) => {
    ...
    return (
      <Screen style={styles.screen} background={renderBackground}>
        <Card style={styles.fallbackCard}>
          <AppText color={TEXT_PRIMARY} style={contentDirectionStyle}>{t('postLoadError')}</AppText>
        </Card>
      </Screen>
    );
  }
  ...
  return (
    ...
    >
      <Card style={styles.heroCard}>
        {postImageUrl ? (
          <Image source={{ uri: postImageUrl }} style={styles.heroImage} resizeMode="cover" />
        ) : (
        ...
        <AppText weight="bold" size="xl" color={TEXT_PRIMARY} style={[styles.postTitle, titleTextStyle]}>
          {post.title}
        </AppText>
        ...
        <View style={styles.metaRow}>
          ...
          {post.category?.name ? (
            <View style={styles.metaChip}>
              <AppText size="xs" weight="semibold" color={TEXT_SECONDARY} style={[styles.metaText, contentDirectionStyle]}>
                {`${labels.category}: ${post.category.name}`}
              </AppText>
            </View>
          ) : null}

          {typeof postViews === 'number' ? (
            <View style={styles.metaChip}>
              <AppText size="xs" weight="semibold" color={TEXT_SECONDARY} style={[styles.metaText, contentDirectionStyle]}>
                {`${labels.views}: ${postViews}`}
              </AppText>
            </View>
          ) : null}
        </View>
